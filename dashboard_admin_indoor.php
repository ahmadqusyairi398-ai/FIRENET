...
/* Badge Styles untuk Indikator Grafik */
.badge {
        ...
        fetch('api_delete_dummy.php?device=indoor')
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    // Refresh grafik secara otomatis
...
function renderLocationMarkers(locations, isDanger) {
    ...
    locations.forEach((loc, idx) => {
        ...
        const isLiveLoc = (rawIdUpper === 'LOK-002' || rawIdUpper === 'IND-002' || loc.id === 2);
        const isDummyLoc = (loc.is_dummy !== undefined) ? (loc.is_dummy == 1 ? 1 : 0) : (isLiveLoc ? 0 : 1);
        const isLocDanger = isLiveLoc ? isDanger : false;
...
async function updateDashboard() {
    ...
    let isDangerDetected = data.isDanger || (data.api === "Terdeteksi Api") || (data.asap === "Tinggi" || data.asap === "Bahaya");
    ...
    updateLocationStatus(isDangerDetected);
