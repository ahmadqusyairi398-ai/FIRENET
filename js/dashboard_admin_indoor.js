/**
 * Script Logika Frontend Dashboard Admin Indoor
...
const pageConfig = window.INDOOR_ADMIN_DATA || {};
...
function renderLocationMarkers(locations, statusText, isDanger) {
    ...
    if (totalElem) totalElem.innerHTML = locations.length;
...
// ================= AUTO CLEAN SAAT HALAMAN DIBUKA =================
    ...
    fetch('api_auto_clean_indoor.php')
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (data.warning) {
                    const banner = document.getElementById('auto-clean-banner');
                    const bannerText = document.getElementById('auto-clean-text');
                    if (banner) banner.style.display = 'flex';
                    if (bannerText) bannerText.innerText = `Terdapat ${data.warning_count} baris data sensor yang usianya menginjak 6 hari. Data tersebut akan terhapus secara permanen besok.`;
                }
            }
        })
